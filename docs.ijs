NB. JOD dictionary dump: 22 Aug 2011 17:05:38
...
HomePIPDocs=:'c:/Documents and Settings/Administrator/My Documents/Ubuntu One/pipdocs/'
...
if. IFWIN do.
  if.        0<#1!:0 <HomePIPDocs,'*.*' do. HomePIPDocs
  elseif.    0<#1!:0 <WorkPIPDocs,'*.*' do. WorkPIPDocs
  elseif.do. ''
  end.
else.
  NB. linux location
  '/home/john/Ubuntu One/pipdocs/'
end.
)
...
NB. referenced files refer to local files in one of three machine
NB. directories - map files to current machine - recompiling 
NB. libnotes.bi will point the resulting pdf to the current local files.
pipdir=. bibLocalPDFs 0
files=. (<pipdir) ,&.> '/pipdocs/'&afterstr&.> files
...
zz=:zz,'InsertDudVersion LF RunDudPreprocess WorkPIPDocs afterstr assert be'
zz=:zz,'forelaststr beforestr bibLocalPDFs bibMendeleyNotes bibitems boxope'
zz=:zz,'n changestr charsub fboxname fexist monthsbetween read showpass tim'
zz=:zz,'estamp today todayno versionymw weekday weeknumber weeksbetween wee'
zz=:zz,'ksinyear write''                                                    '
zz=:350{.zz
